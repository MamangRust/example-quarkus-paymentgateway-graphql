...
public class TransactionMethodImplService implements TransactionMethodService {
    ...
    public TransactionMethodImplService(TransactionMethodRepository transactionMethodRepository,
        ...
        this.tracer = openTelemetry.getTracer("transaction-method-service", "1.0.0");
        Meter meter = openTelemetry.getMeter("transaction-method-service");
    ...
    @Override
    public Uni<ApiResponse<List<TransactionMonthMethodResponse>>> findMonthlyMethod(Long year) {
        logger.info("📊 Fetching monthly transaction methods for year={}", year);
        ...
        String cacheKey = String.format("transactions:stats:method:month:%d", year);
                ...
                .chain(cachedJson -> {
                    if (cachedJson != null) {
                        ...
                        return Uni.createFrom().item(ApiResponse
                                .success("Successfully fetched monthly transaction methods for year=" + year, cached));
                    }

                    logger.info("Cache MISS for key: {}. Fetching from DB.", cacheKey);
                    long startTime = System.currentTimeMillis();
                    Span span = tracer.spanBuilder("findMonthlyMethod")
                            .setSpanKind(SpanKind.SERVER)
                            .setAttribute("service.name", "transaction-method-service")
                            .setAttribute("operation", "find_monthly_method")
                            .setAttribute("year", String.valueOf(year))
                            .startSpan();

                    return transactionMethodRepository.findMonthlyPaymentMethods(year)
                            .chain(methods -> {
                                List<TransactionMonthMethodResponse> response = methods.stream()
                                ...
                                return redisService
                                        ...
                                        .map(v -> {
                                            ...
                                            requestsTotal.add(1, Attributes.of(
                                                    AttributeKey.stringKey("operation"), "find_monthly_method",
                                                    AttributeKey.stringKey("status"), "success"));

                                            return ApiResponse.success(
                                                    "Successfully fetched monthly transaction methods for year=" + year,
                                                    response);
                                        });
                            })
                            .onFailure().recoverWithItem(e -> {
                                logger.error("💥 Failed to fetch monthly transaction methods for year={}", year, e);
                                ...
                                return new ApiResponse<>("error",
                                        "Failed to fetch monthly transaction methods for year=" + year,
    ...
    @Override
    public Uni<ApiResponse<List<TransactionYearMethodResponse>>> findYearlyMethod(Long year) {
        logger.info("📊 Fetching yearly transaction methods until year={}", year);
        ...
        String cacheKey = String.format("transactions:stats:method:year:%d", year);
        ...
        return redisService.getReactive(cacheKey)
                .chain(cachedJson -> {
                    if (cachedJson != null) {
                        ...
                        return Uni.createFrom().item(ApiResponse
                                .success("Successfully fetched yearly transaction methods until year=" + year, cached));
                    }

                    logger.info("Cache MISS for key: {}. Fetching from DB.", cacheKey);
                    long startTime = System.currentTimeMillis();
                    Span span = tracer.spanBuilder("findYearlyMethod")
                            .setSpanKind(SpanKind.SERVER)
                            .setAttribute("service.name", "transaction-method-service")
                    ...
                    return transactionMethodRepository.findYearlyPaymentMethods(year)
                            .chain(methods -> {
                                List<TransactionYearMethodResponse> response = methods.stream()
                                ...
                                return redisService
                                        ...
                                        .map(v -> {
                                            span.setStatus(StatusCode.OK);
                                            requestsTotal.add(1, Attributes.of(
                                                    AttributeKey.stringKey("operation"), "find_yearly_method",
                                                    AttributeKey.stringKey("status"), "success"));

                                            return ApiResponse.success(
                                                    "Successfully fetched yearly transaction methods until year="
                                                            + year,
                                                    response);
                                        });
                            })
                            .onFailure().recoverWithItem(e -> {
                                logger.error("💥 Failed to fetch yearly transaction methods until year={}", year, e);
                                span.recordException(e);
                                span.setStatus(StatusCode.ERROR, e.getMessage());
                                ...
                                return new ApiResponse<>("error",
                                        "Failed to fetch yearly transaction methods until year=" + year,
                                        Collections.emptyList());
                            })
                            .eventually(() -> {
                                span.end();
