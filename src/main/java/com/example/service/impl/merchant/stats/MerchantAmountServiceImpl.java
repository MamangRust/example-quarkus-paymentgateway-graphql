...
import com.example.repository.merchant.stats.MerchantAmountRepository;
...
@ApplicationScoped
public class MerchantAmountServiceImpl implements MerchantAmountService {
    private static final Logger logger = LoggerFactory.getLogger(MerchantAmountServiceImpl.class);

    private final MerchantAmountRepository merchantAmountRepository;
    ...
    @Inject
    public MerchantAmountServiceImpl(MerchantAmountRepository merchantAmountRepository,
            OpenTelemetry openTelemetry,
            RedisService redisService,
            ObjectMapper objectMapper) {
        this.merchantAmountRepository = merchantAmountRepository;
        this.redisService = redisService;
        this.objectMapper = objectMapper;
        this.tracer = openTelemetry.getTracer("merchant-amount-service", "1.0.0");
        Meter meter = openTelemetry.getMeter("merchant-amount-service");
    ...
    @Override
    public Uni<ApiResponse<List<MerchantResponseMonthlyAmount>>> findMonthAmount(Long year) {
        try {
            validateYear(year);
        ...
        String cacheKey = "merchant-stats:amount:monthly:" + year;
        ...
        return redisService.getReactive(cacheKey)
                .chain(cachedJson -> {
                    ...
                    Span span = tracer.spanBuilder("findMonthAmount")
                            ...
                            .setAttribute("service.name", "merchant-amount-service")
                            .setAttribute("operation", "find_month_amount")
                            .setAttribute("year", year.toString())
                            .startSpan();

                    return merchantAmountRepository.findMonthlyAmount(year)
                            .chain(monthlyAmounts -> {
                                ...
                                return redisService
                                        ...
                                        .map(v -> {
                                            logger.info("Cached monthly merchant amounts for year: {}", year);
                                            span.setStatus(StatusCode.OK);

                                            requestsTotal.add(1, Attributes.of(
                                                    AttributeKey.stringKey("operation"), "find_month_amount",
                                                    AttributeKey.stringKey("status"), "success"));
                                            return response;
                                        });
                            })
                            .onFailure().invoke(e -> {
                                logger.error("❌ Failed to fetch monthly merchant amounts for year={}", year, e);
    ...
    @Override
    public Uni<ApiResponse<List<MerchantResponseYearlyAmount>>> findYearAmount(Long year) {
        try {
            validateYear(year);
        } catch (IllegalArgumentException e) {
            return Uni.createFrom().item(new ApiResponse<>("error", e.getMessage(), java.util.Collections.emptyList()));
        ...
        String cacheKey = "merchant-stats:amount:yearly:" + year;
        ...
        return redisService.getReactive(cacheKey)
                .chain(cachedJson -> {
                    ...
                    return merchantAmountRepository.findYearlyAmount(year)
                            .chain(yearlyAmounts -> {
                                ...
                                return redisService
                                        .setWithExpirationReactive(cacheKey, toJson(response), STATS_CACHE_TTL_SECONDS)
                                        .map(v -> {
                                            logger.info("Cached yearly merchant amounts for year: {}", year);
                                            span.setStatus(StatusCode.OK);

                                            requestsTotal.add(1, Attributes.of(
                                                    AttributeKey.stringKey("operation"), "find_year_amount",
                                                    AttributeKey.stringKey("status"), "success"));
                                            return response;
                                        });
                            })
                            .onFailure().invoke(e -> {
                                logger.error("❌ Failed to fetch yearly merchant amounts for year={}", year, e);
                                span.recordException(e);
                                span.setStatus(StatusCode.ERROR, e.getMessage());
