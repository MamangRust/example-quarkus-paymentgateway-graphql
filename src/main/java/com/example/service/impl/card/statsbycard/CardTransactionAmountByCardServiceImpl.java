...
import com.example.repository.card.statsbycard.CardTransactionAmountByCardRepository;
...
@ApplicationScoped
public class CardTransactionAmountByCardServiceImpl implements CardTransactionAmountByCardService {
        private static final Logger logger = LoggerFactory.getLogger(CardTransactionAmountByCardServiceImpl.class);

        private final CardTransactionAmountByCardRepository cardTransactionAmountByCardRepository;
        ...
        @Inject
        public CardTransactionAmountByCardServiceImpl(
                        CardTransactionAmountByCardRepository cardTransactionAmountByCardRepository,
                        OpenTelemetry openTelemetry,
                        RedisService redisService,
                        ObjectMapper objectMapper) {
                this.cardTransactionAmountByCardRepository = cardTransactionAmountByCardRepository;
                this.redisService = redisService;
                this.objectMapper = objectMapper;
                this.tracer = openTelemetry.getTracer("card-transaction-amount-by-card-service", "1.0.0");
                Meter meter = openTelemetry.getMeter("card-transaction-amount-by-card-service");
        ...
        public Uni<ApiResponse<List<CardResponseMonthAmount>>> findMonthAmountByCard(MonthYearCardNumberCard req) {
                ...
                String cacheKey = "card-transaction:" + req.getCardNumber() + ":month:" + req.getYear();
                                ...
                                .chain(cachedJson -> {
                                        ...
                                        Span span = tracer.spanBuilder("findMonthAmountByCard")
                                                        ...
                                                        .setAttribute("service.name",
                                                                        "card-transaction-amount-by-card-service")
                                        ...
                                        return cardTransactionAmountByCardRepository
                                                        .getMonthlyTransactionAmountByCard(req.getCardNumber(),
                                                                        yearDate)
                                                        .chain(balances -> {
                                                                if (balances.isEmpty()) {
                                                                        logger.warn("⚠️ No monthly transaction data found for card={} year={}",
                                                                                        req.getCardNumber(),
                                                                                        req.getYear());
                                                                        ApiResponse<List<CardResponseMonthAmount>> response = new ApiResponse<>(
                                                                                        "error",
                                                                                        "No monthly transaction stats found for card "
                                                                                                        + req.getCardNumber()
                                                                                                        + " year "
                                                                                                        + req.getYear(),
                                                                                        List.of());
                                                        ...
                                                        .onFailure().invoke(e -> {
                                                                logger.error("❌ Failed to fetch monthly transaction for card={} year={}",
        ...
        public Uni<ApiResponse<List<CardResponseYearAmount>>> findYearAmountByCard(MonthYearCardNumberCard req) {
                ...
                String cacheKey = "card-transaction:" + req.getCardNumber() + ":year:" + req.getYear();
                ...
                return redisService.getReactive(cacheKey)
                                .chain(cachedJson -> {
                                        ...
                                        return cardTransactionAmountByCardRepository
                                                        .getYearlyTransactionAmountByCard(req.getCardNumber(),
                                                                        req.getYear())
                                                        .chain(balances -> {
                                                                if (balances.isEmpty()) {
                                                                        logger.warn("⚠️ No yearly transaction data found for card={} year={}",
                                                                                        req.getCardNumber(),
                                                                                        req.getYear());
                                                                        ApiResponse<List<CardResponseYearAmount>> response = new ApiResponse<>(
                                                                                        "error",
                                                                                        "No yearly transaction stats found for card "
                                                                                                        + req.getCardNumber()
                                                                                                        + " year "
                                                                                                        + req.getYear(),
                                                                                        List.of());
                                                        ...
                                                        .onFailure().invoke(e -> {
                                                                logger.error("❌ Failed to fetch yearly transaction for card={} year={}",
                                                                                req.getCardNumber(), req.getYear(), e);
                                                                span.recordException(e);
                                                                span.setStatus(StatusCode.ERROR, e.getMessage());
