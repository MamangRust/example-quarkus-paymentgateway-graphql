...
@ApplicationScoped
public class WithdrawAmountByCardRepository implements PanacheRepository<Withdraw> {

    public Uni<List<WithdrawMonthlyAmount>> findMonthlyWithdrawsByCard(String cardNumber, Long year) {
        String sql = """
                WITH months AS (
        ...
        return getSession().chain(session -> session.createNativeQuery(sql, Object[].class)
                ...
                .map(list -> list.stream()
                        .map(row -> new WithdrawMonthlyAmount(
                                (String) row[0],
                                row[1] == null ? 0L : ((Number) row[1]).longValue()))
                        .toList()));
    }

    public Uni<List<WithdrawYearlyAmount>> findYearlyWithdrawsByCard(String cardNumber, Long year) {
        String sql = """
                WITH years AS (
        ...
        return getSession().chain(session -> session.createNativeQuery(sql, Object[].class)
                ...
                .getResultList()
                .map(list -> list.stream()
                        .map(row -> new WithdrawYearlyAmount(
