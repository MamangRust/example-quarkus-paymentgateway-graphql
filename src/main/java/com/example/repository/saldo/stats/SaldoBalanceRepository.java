...
@ApplicationScoped
public class SaldoBalanceRepository implements PanacheRepository<Saldo> {

    public Uni<List<SaldoMonthBalance>> findMonthlySaldoBalances(Long year) {
        String sql = """
                WITH months AS (
                ...
                FROM months m
                LEFT JOIN saldos s ON EXTRACT(MONTH FROM s.created_at) = m.m
        ...
        return getSession().chain(session -> session.createNativeQuery(sql, Object[].class)
                ...
                .map(list -> list.stream()
                        .map(row -> new SaldoMonthBalance(
                                (String) row[0],
                                row[1] == null ? 0L : ((Number) row[1]).longValue()))
                        .toList()));
    }

    public Uni<List<SaldoYearBalance>> findYearlySaldoBalances(Long year) {
        String sql = """
                WITH years AS (
                ...
                SELECT
                    ...
                    CAST(COALESCE(SUM(s.total_balance), 0) AS BIGINT) AS totalBalance
                FROM years y
                LEFT JOIN saldos s ON EXTRACT(YEAR FROM s.created_at) = y.y
                    AND s.deleted_at IS NULL
                GROUP BY y.y
                ORDER BY y.y
                """;
