...
@ApplicationScoped
public class CardTransactionAmountRepository implements PanacheRepository<Card> {

    public Uni<List<CardMonthAmount>> getMonthlyTransactionAmount(LocalDate yearDate) {
        String sql = """
                WITH months AS (
                ...
                LEFT JOIN
                    ...
                    AND EXTRACT(YEAR FROM t.transaction_time) = EXTRACT(YEAR FROM :yearDate)
    ...
    public Uni<List<CardYearAmount>> getYearlyTransactionAmount(Long year) {
        String sql = """
                WITH years AS (
                ...
                FROM
                    years y
                LEFT JOIN
                    transactions t ON EXTRACT(YEAR FROM t.transaction_time) = y.y
                    AND t.deleted_at IS NULL
                LEFT JOIN
                    cards c ON t.card_number = c.card_number
                    AND c.deleted_at IS NULL
