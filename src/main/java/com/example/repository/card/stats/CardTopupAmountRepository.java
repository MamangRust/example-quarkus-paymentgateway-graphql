...
@ApplicationScoped
public class CardTopupAmountRepository implements PanacheRepository<Card> {

    public Uni<List<CardMonthAmount>> getMonthlyTopupAmount(LocalDate yearDate) {
        String sql = """
                WITH months AS (
    ...
    public Uni<List<CardYearAmount>> getYearlyTopupAmount(Long year) {
        String sql = """
                WITH years AS (
                ...
                FROM
                    years y
                LEFT JOIN
                    topups t ON EXTRACT(YEAR FROM t.topup_time) = y.y
                    AND t.deleted_at IS NULL
                LEFT JOIN
                    cards c ON t.card_number = c.card_number
                    AND c.deleted_at IS NULL
