...
@ApplicationScoped
public class CardTopupAmountByCardRepository implements PanacheRepository<Card> {

    public Uni<List<CardMonthAmount>> getMonthlyTopupAmountByCard(String cardNumber, LocalDate yearDate) {
        String sql = """
                WITH months AS (
                ...
                FROM
                    months m
                LEFT JOIN
                    topups t ON EXTRACT(MONTH FROM t.topup_time) = m.m
                    AND EXTRACT(YEAR FROM t.topup_time) = EXTRACT(YEAR FROM :yearDate)
    ...
    public Uni<List<CardYearAmount>> getYearlyTopupAmountByCard(String cardNumber, Long year) {
        String sql = """
                WITH years AS (
                ...
                JOIN
                    cards c ON t.card_number = c.card_number
                RIGHT JOIN
                    years y ON EXTRACT(YEAR FROM t.topup_time) = y.y
                      AND t.deleted_at IS NULL
                      AND c.deleted_at IS NULL
                      AND t.card_number = :cardNumber
                GROUP BY
