...
@ApplicationScoped
public class CardBalanceByCardRepository implements PanacheRepository<Card> {

    public Uni<List<CardMonthBalance>> getMonthlyBalancesByCard(LocalDate yearDate, String cardNumber) {
        String sql = """
                WITH months AS (
                ...
                FROM months m
                LEFT JOIN saldos s
                    ON EXTRACT(MONTH FROM s.created_at) = m.m
                    AND EXTRACT(YEAR FROM s.created_at) = EXTRACT(YEAR FROM CAST(:yearDate AS date))
    ...
    public Uni<List<CardYearBalance>> getYearlyBalancesByCard(Long year, String cardNumber) {
        String sql = """
                WITH yearly_data AS (
                    SELECT
