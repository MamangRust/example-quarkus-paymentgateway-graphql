...
@ApplicationScoped
public class TransactionStatusRepository implements PanacheRepository<Transaction> {

    public Uni<List<TransactionMonthStatusSuccess>> findMonthTransactionStatusSuccess(
    ...
    public Uni<List<TransactionMonthStatusFailed>> findMonthTransactionStatusFailed(
            Long startYear, Integer startMonth, Long endYear, Integer endMonth) {
        String sql = """
                WITH months AS (
                    SELECT 1 AS mon UNION ALL SELECT 2 UNION ALL SELECT 3 UNION ALL SELECT 4 UNION ALL
    ...
    public Uni<List<TransactionYearStatusSuccess>> findYearlyTransactionStatusSuccess(Long year) {
        String sql = """
                WITH yearly_data AS (
                    SELECT
                    ...
                    UNION ALL
                    SELECT CAST(:year AS VARCHAR), 0, 0 WHERE NOT EXISTS (SELECT 1 FROM yearly_data WHERE reportYear = CAST(:year AS VARCHAR))
                    UNION ALL
                    SELECT CAST(:year - 1 AS VARCHAR), 0, 0 WHERE NOT EXISTS (SELECT 1 FROM yearly_data WHERE reportYear = CAST(:year - 1 AS VARCHAR))
                )
    ...
    public Uni<List<TransactionYearStatusFailed>> findYearlyTransactionStatusFailed(Long year) {
        String sql = """
                WITH yearly_data AS (
                    SELECT
                        CAST(EXTRACT(YEAR FROM t.transaction_time) AS VARCHAR) AS reportYear,
