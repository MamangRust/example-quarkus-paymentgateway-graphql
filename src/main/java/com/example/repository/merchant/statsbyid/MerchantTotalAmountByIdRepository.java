...
@ApplicationScoped
public class MerchantTotalAmountByIdRepository implements PanacheRepository<Merchant> {

    public Uni<List<MerchantMonthlyTotalAmount>> findMonthlyTotalAmountById(Long merchantId, LocalDate date) {
        String sql = """
    ...
    public Uni<List<MerchantYearlyTotalAmount>> findYearlyTotalAmountById(Long merchantId, Long year,
            Long previousYear) {
        String sql = """
                WITH yearly_data AS (
                    SELECT
