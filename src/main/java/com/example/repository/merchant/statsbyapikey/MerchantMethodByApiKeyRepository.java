package com.example.repository.merchant.statsbyapikey;
...
@ApplicationScoped
public class MerchantMethodByApiKeyRepository implements PanacheRepository<Merchant> {

    public Uni<List<MerchantMonthlyPaymentMethod>> findMonthlyPaymentMethodsByApiKey(String apiKey, Long year) {
        String sql = """
                WITH months AS (
                ...
                payment_methods AS (
                    ...
                    FROM transactions t
                    INNER JOIN merchants mch ON t.merchant_id = mch.merchant_id
                    WHERE t.deleted_at IS NULL
                      AND mch.deleted_at IS NULL
                      AND mch.api_key = :apiKey
                )
                SELECT
                    CASE m.m
    ...
    public Uni<List<MerchantYearlyPaymentMethod>> findYearlyPaymentMethodsByApiKey(String apiKey, Long endYear) {
        String sql = """
                WITH years AS (
